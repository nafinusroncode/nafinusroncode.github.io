...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      ...
      focus:outline-none focus:ring-2 focus:ring-[#073F6F] focus:ring-offset-2
      focus:ring-offset-[#061A2F] transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(7,63,111,0.4)] border-[#073F6F]"
      : "border-white/20 hover:border-[#073F6F]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#073F6F]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#073F6F] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#073F6F]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
