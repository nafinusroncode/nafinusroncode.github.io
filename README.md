...
```bash
git clone https://github.com/nafinusroncode/portfolio.git
cd Portofolio_V5
npm install
```
...
This project is ready to deploy with GitHub Actions. The workflow is available at `.github/workflows/deploy.yml`.

Recommended repository options:

- Use `nafinusroncode.github.io` for a user site at `https://nafinusroncode.github.io/`.
- Use any repository name if you will connect the custom domain `https://nafinusroncode.com`.
...
4. Push to the `main` branch, then wait for the workflow to finish in the **Actions** tab.

For `https://nafinusroncode.com`, buy/configure the domain first, then add it in **Settings -> Pages -> Custom domain** and update DNS at the domain provider.
